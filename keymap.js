let Key;
(function (Key) {
  Key[Key.Backspace = 8] = 'Backspace'
  Key[Key.Tab = 9] = 'Tab'
  Key[Key.Enter = 13] = 'Enter'
  Key[Key.LeftShift = 16] = 'LeftShift'
  Key[Key.RightShift = 16] = 'LeftShift'
  Key[Key.LeftControl = 17] = 'LeftControl'
  Key[Key.RightControl = 17] = 'LeftControl'
  Key[Key.LeftAlt = 18] = 'LeftAlt'
  Key[Key.RightAlt = 18] = 'LeftAlt'
  Key[Key.PauseBreak = 19] = 'PauseBreak'
  Key[Key.CapsLock = 20] = 'CapsLock'
  Key[Key.Escape = 27] = 'Escape'
  Key[Key.Space = 32] = 'Space'
  Key[Key.PageUp = 33] = 'PageUp'
  Key[Key.PageDown = 34] = 'PageDown'
  Key[Key.End = 35] = 'End'
  Key[Key.Home = 36] = 'Home'
  Key[Key.Left = 37] = 'Left'
  Key[Key.Up = 38] = 'Up'
  Key[Key.Right = 39] = 'Right'
  Key[Key.Down = 40] = 'Down'
  Key[Key.Insert = 45] = 'Insert'
  Key[Key.Delete = 46] = 'Delete'
  Key[Key.Num0 = 48] = 'Num0'
  // Key[Key.ClosedParen = 48] = 'ClosedParen'
  Key[Key.Num1 = 49] = 'Num1'
  // Key[Key.ExclamationMark = 49] = 'ExclamationMark'
  Key[Key.Num2 = 50] = 'Num2'
  // Key[Key.AtSign = 50] = 'AtSign'
  Key[Key.Num3 = 51] = 'Num3'
  // Key[Key.PoundSign = 51] = 'PoundSign'
  // Key[Key.Hash = 51] = 'Hash'
  Key[Key.Num4 = 52] = 'Num4'
  // Key[Key.DollarSign = 52] = 'DollarSign'
  Key[Key.Num5 = 53] = 'Num5'
  // Key[Key.PercentSign = 53] = 'PercentSign'
  Key[Key.Num6 = 54] = 'Num6'
  // Key[Key.Caret = 54] = 'Caret'
  // Key[Key.Hat = 54] = 'Hat'
  Key[Key.Num7 = 55] = 'Num7'
  // Key[Key.Ampersand = 55] = 'Ampersand'
  Key[Key.Num8 = 56] = 'Num8'
  // Key[Key.Star = 56] = 'Star'
  // Key[Key.Asterik = 56] = 'Asterik'
  Key[Key.Num9 = 57] = 'Num9'
  // Key[Key.OpenParen = 57] = 'OpenParen'
  Key[Key.A = 65] = 'A'
  Key[Key.B = 66] = 'B'
  Key[Key.C = 67] = 'C'
  Key[Key.D = 68] = 'D'
  Key[Key.E = 69] = 'E'
  Key[Key.F = 70] = 'F'
  Key[Key.G = 71] = 'G'
  Key[Key.H = 72] = 'H'
  Key[Key.I = 73] = 'I'
  Key[Key.J = 74] = 'J'
  Key[Key.K = 75] = 'K'
  Key[Key.L = 76] = 'L'
  Key[Key.M = 77] = 'M'
  Key[Key.N = 78] = 'N'
  Key[Key.O = 79] = 'O'
  Key[Key.P = 80] = 'P'
  Key[Key.Q = 81] = 'Q'
  Key[Key.R = 82] = 'R'
  Key[Key.S = 83] = 'S'
  Key[Key.T = 84] = 'T'
  Key[Key.U = 85] = 'U'
  Key[Key.V = 86] = 'V'
  Key[Key.W = 87] = 'W'
  Key[Key.X = 88] = 'X'
  Key[Key.Y = 89] = 'Y'
  Key[Key.Z = 90] = 'Z'
  Key[Key.LeftSuper = 91] = 'LeftSuper'
  Key[Key.RightSuper = 93] = 'RightSuper'
  Key[Key.NumPad0 = 96] = 'NumPad0'
  Key[Key.NumPad1 = 97] = 'NumPad1'
  Key[Key.NumPad2 = 98] = 'NumPad2'
  Key[Key.NumPad3 = 99] = 'NumPad3'
  Key[Key.NumPad4 = 100] = 'NumPad4'
  Key[Key.NumPad5 = 101] = 'NumPad5'
  Key[Key.NumPad6 = 102] = 'NumPad6'
  Key[Key.NumPad7 = 103] = 'NumPad7'
  Key[Key.NumPad8 = 104] = 'NumPad8'
  Key[Key.NumPad9 = 105] = 'NumPad9'
  Key[Key.Multiply = 106] = 'Multiply'
  Key[Key.Add = 107] = 'Add'
  Key[Key.Subtract = 109] = 'Subtract'
  Key[Key.DecimalPoint = 110] = 'DecimalPoint'
  Key[Key.Divide = 111] = 'Divide'
  Key[Key.F1 = 112] = 'F1'
  Key[Key.F2 = 113] = 'F2'
  Key[Key.F3 = 114] = 'F3'
  Key[Key.F4 = 115] = 'F4'
  Key[Key.F5 = 116] = 'F5'
  Key[Key.F6 = 117] = 'F6'
  Key[Key.F7 = 118] = 'F7'
  Key[Key.F8 = 119] = 'F8'
  Key[Key.F9 = 120] = 'F9'
  Key[Key.F10 = 121] = 'F10'
  Key[Key.F11 = 122] = 'F11'
  Key[Key.F12 = 123] = 'F12'
  Key[Key.NumLock = 144] = 'NumLock'
  Key[Key.ScrollLock = 145] = 'ScrollLock'
  Key[Key.Semicolon = 186] = 'Semicolon'
  Key[Key.Equals = 187] = 'Equals'
  Key[Key.Comma = 188] = 'Comma'
  Key[Key.Subtract = 189] = 'Subtract'
  Key[Key.Period = 190] = 'Period'
  // Key[Key.UnderScore = 189] = 'UnderScore'
  Key[Key.Add = 187] = 'Add'
  Key[Key.Slash = 191] = 'Slash'
  // Key[Key.Tilde = 192] = 'Tilde'
  Key[Key.Grave = 192] = 'Grave'
  Key[Key.LeftBracket = 219] = 'LeftBracket'
  Key[Key.Backslash = 220] = 'Backslash'
  Key[Key.RightBracket = 221] = 'RightBracket'
  Key[Key.Quote = 222] = 'Quote'
})(Key = exports.Key || (exports.Key = {}))
